...
//                ->_addLeft($this->getLayout()->createBlock('qquoteadv/adminhtml_qquoteadv_edit_tabs'));
    ...
    public function saveAction()
    {
        ...
        if ($event) {
            Mage::dispatchEvent('ophirah_qquoteadv_admin_save_before_' . $event, array($data));
        }
            ...
            try {
                if (isset($data['product']) && is_array($data['product']) && count($data['product']) > 0) {
                    ...
                    if ($quoteId) {
                        ...
                        $event = $this->getRequest()->getParam('event');
                        if ($event) {
                            Mage::dispatchEvent('qquoteadv_qqadvcustomer_before_saveaction_' . $event, array('quote' => $_quoteadv));
                        }
...
//                            }
                        ...
                        if ($hold) {
                            ...
                            $event = $this->getRequest()->getParam('event');
                            if ($event) {
                                Mage::dispatchEvent('qquoteadv_qqadvcustomer_aftersave_final_' . $event, array('quote' => $_quoteadv));
                                Mage::dispatchEvent('qquoteadv_qqadvcustomer_before_saveaction_' . $event, array('quote' => $_quoteadv));
                            }
                        }
                    }
                ...
                if (count(Mage::getSingleton('adminhtml/session')->getMessages()->getErrors())) {
                    ...
                    $event = $this->getRequest()->getParam('event');
                    if ($event) {
                        Mage::dispatchEvent('qquoteadv_qqadvcustomer_aftersave_final_error_' . $event, array('quote' => $_quoteadv));
                    }
                } else {
                    ...
                    $event = $this->getRequest()->getParam('event');
                    if ($event) {
                        Mage::dispatchEvent('qquoteadv_qqadvcustomer_aftersave_final_success_' . $event, array('quote' => $_quoteadv));
                    }
                }
        ...
        $event = $this->getRequest()->getParam('event');
        if ($event) {
            Mage::dispatchEvent('ophirah_qquoteadv_admin_save_after_' . $event, array($data));
        }
    ...
    public function sendEmailSalesRep($_quoteadv, $saveas)
    {
        ...
        if (!empty($emailAddresses)) {
            ...
            if ($quoteadv_param != $disabledEmail && count($emailAddresses) > 0) {
                ...
                $vars = array(
                    'quoteUrl' => Mage::helper("adminhtml")->getUrl("adminhtml/redirect/quoteEdit", array('id' => $_quoteadv->getId())),
    ...
    public function getMsgToUpgrade($updateMsg = false, $storeId = 0)
    {
        ...
        //if ((true)) {
            ...
            $btn1 = '<button target="_blank" class="button button2 floatleft" title="Contact Sales" onclick="document.location.href=\'https://cart2quotesales.zendesk.com/hc/en-us/requests/new\'">' . $this->__('Contact Sales') . '</button> ';
            $btn2 = '<button target="_blank" class="button button2 btn-prim" title="Editions and Pricing" onclick="document.location.href=\'https://www.cart2quote.com/magento-quotation-module-plans-pricing.html\'">' . $this->__('Editions and Pricing') . '</button> ';
    ...
    protected function _convertQuoteItemsToOrder($quoteadvId, $requestedItems)
    {
        ...
        foreach ($data as $item) {
            ...
            if ($defaultStoreId != $quoteStoreId) {
                $priceContainsTax = Mage::helper('tax')
                    ->priceIncludesTax($quoteStoreId); //Mage::getStoreConfig('tax/calculation/price_includes_tax', $quoteStoreId);
                if ($priceContainsTax) {
                    //fallback for situations where getWebsite doesn't return a object
                    if (is_object(Mage::app()->getWebsite(true))) {
                        $store = Mage::app()->getWebsite(true)->getDefaultGroup()->getDefaultStore();
    ...
    public function convertAction()
    {
        ...
        if (!empty($quoteadvId)) {
            ...
            Mage::dispatchEvent(
                ...
                array('orderCreateModel' => $this->_getOrderCreateModel())
            );
    ...
    public function swith2QuoteAction()
    {
        ...
        if (($billingAddress->getData('save_in_address_book') == 1) && ($billingAddress->getData('customer_address_id') == '0')) {
            $helperAddress->addQuoteAddress($customerId, $billingAddress->exportCustomerAddress()->getData());
        } elseif ($billingAddress->getData('save_in_address_book') == 1) {
            $helperAddress->updateQuoteAddress($customerId, $customerBillingAddressId, $billingAddress->exportCustomerAddress()->getData());
        }

        //Save or update shipping address
        if (($shipAddress->getData('save_in_address_book') == 1) && ($shipAddress->getData('customer_address_id') == '0')) {
            $helperAddress->addQuoteAddress($customerId, $shipAddress->exportCustomerAddress()->getData());
        } elseif ($shipAddress->getData('save_in_address_book') == 1) {
            $helperAddress->updateQuoteAddress($customerId, $customerShipAddressId, $shipAddress->exportCustomerAddress()->getData());
        }
    ...
    public function testMagentoMailAction()
    {
        ...
        try {
            ...
            $template->setData('c2qParams', array('email' => $testEmailAddress, 'name' => $senderName));
            Mage::dispatchEvent('ophirah_qquoteadv_addSendMail_before', array('template' => $template));
            $result = $template->send($testEmailAddress, $senderName, array());
            Mage::dispatchEvent('ophirah_qquoteadv_addSendMail_after', array('template' => $template, 'result' => $result));
            if ($result) {
