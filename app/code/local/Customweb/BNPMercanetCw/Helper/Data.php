...
class Customweb_BNPMercanetCw_Helper_Data extends Mage_Core_Helper_Abstract
{
	private static $container = null;

	public function log($message, $level = null, $file = '', $forceLog = false) {
		if (Mage::getStoreConfig('bnpmercanetcw/general/debug_log') == '1') {
	...
	public function createContainer() {
		if (self::$container === null) {
			...
			$packages[] = 'Customweb_BNPMercanetCw_Model_';
	...
	public function getAssetResolver() {
		return new Customweb_Asset_Resolver_Composite(array(
			Mage::getModel('bnpmercanetcw/asset_skinResolver'),
			Mage::getModel('bnpmercanetcw/asset_jsResolver'),
			Mage::getModel('bnpmercanetcw/asset_templateResolver'),
			new Customweb_Asset_Resolver_Simple(Mage::getBaseDir('media') . '/customweb/bnpmercanetcw/assets/', Mage::getBaseUrl('media') . '/customweb/bnpmercanetcw/assets/')
		));
	}
	...
	{
		$transaction = Mage::getModel('bnpmercanetcw/transaction')->load($orderPaymentId, 'order_payment_id');
	...
	{
		$transaction = Mage::getModel('bnpmercanetcw/transaction')->load($orderId, 'order_id');
		if ($transaction !== null && $transaction->getId()) {
			return $transaction;
		}
	...
	{
		$transaction = Mage::getModel('bnpmercanetcw/transaction')->load($transactionId);
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		$transaction = Mage::getModel('bnpmercanetcw/transaction')->load($transactionId, 'transaction_external_id');
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		$transaction = Mage::getModel('bnpmercanetcw/transaction')->load($paymentId, 'payment_id');
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		if ($customer->getData('bnpmercanetcw_migrated')) {
			return;
		}
		...
		$payments = Mage::getModel('sales/order_payment')->getCollection()
			...
			->addFieldToFilter('method', array('like' => 'bnpmercanetcw_%'));
		...
		$query = 'UPDATE ' . Mage::getSingleton('core/resource')->getTableName('customer_entity')
			. ' SET bnpmercanetcw_migrated = 1, created_at = ' . $connection->quote(Varien_Date::formatDate($customer->getCreatedAt()))
	...
	protected function migrateTransaction(Mage_Sales_Model_Order $order)
	{
		if ($order !== null && $order->getPayment() !== false) {
			...
			if (!empty($additionalData)) {
				...
				$alias = Mage::getModel('bnpmercanetcw/aliasdata')->load($order->getId(), 'order_id');
	...
	public function getPaymentCustomerContext($customerId = null)
	{
		...
		return Customweb_BNPMercanetCw_Model_PaymentCustomerContext::getByCustomerId($id);
	}
	...
	{
		$configAdapter = new Customweb_BNPMercanetCw_Model_ConfigurationAdapter();
		return $configAdapter->getConfigurationValue($key);
	}

	public function setConfigurationStoreId($storeId)
	{
		Customweb_BNPMercanetCw_Model_ConfigurationAdapter::setStoreId($storeId);
	}
	...
	{
		if (Mage::getStoreConfig('bnpmercanetcw/general/wait_for_success') != '1') {
