...
class Customweb_BNPMercanetCw_Model_MotoObserver
{
	/**
	...
	public function postOrderSave(Varien_Event_Observer $observer)
	{
		...
		if ($orderId) {
			...
			if (strpos($payment->getMethodInstance()
				->getCode(), 'bnpmercanetcw') !== false) {
				Mage::helper('BNPMercanetCw')->log("Codes match.");
				if ($messageType == 'success') {
					$order->addStatusToHistory(Customweb_BNPMercanetCw_Model_Method::BNPMERCANETCW_STATUS_PENDING, Mage::helper('BNPMercanetCw')->__('Payment is pending at BNPMercanetCw'));
					$order->save();
					Mage::helper('BNPMercanetCw')->log("Redirect to Moto authorization");
					...
					$controller->getResponse()
						->setRedirect(Mage::helper('adminhtml')->getUrl('*/motobnpmercanetcw/process', array(
	...
	public function beforeOrderSave(Varien_Event_Observer $observer)
	{
		try {
			...
			if (isset($_POST['payment']['method'])) {
				...
				if (strpos($paymentCode, 'bnpmercanetcw') !== false) {
