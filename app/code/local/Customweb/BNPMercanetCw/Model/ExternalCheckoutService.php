 ...
 * @Bean
 */
class Customweb_BNPMercanetCw_Model_ExternalCheckoutService implements Customweb_Payment_ExternalCheckout_ICheckoutService {
	...
	public function loadContext($contextId, $cache = true){
		return Mage::getModel('bnpmercanetcw/externalCheckoutContext')->load($contextId);
	}

	public function createSecurityToken(Customweb_Payment_ExternalCheckout_IContext $context){
		if (!($context instanceof Customweb_BNPMercanetCw_Model_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('Customweb_BNPMercanetCw_Model_ExternalCheckoutContext');
		}
	...
	public function authenticate(Customweb_Payment_ExternalCheckout_IContext $context, $emailAddress, $successUrl){
		...
		if ($quote->isAllowedGuestCheckout() && Mage::getStoreConfig('bnpmercanetcw/general/external_checkout_account_creation') == 'skip_selection' && !empty($emailAddress)) {
			$customerData = array(
				'email' => $emailAddress,
			...
			if ($dateOfBirth != null) {
				$customerData['dob'] = Mage::helper('BNPMercanetCw/externalCheckout')->getDateAsString($dateOfBirth);
			}
			...
			$result = Mage::helper('BNPMercanetCw/externalCheckout')->validateCustomerData($quote, $customerData, 'guest');
			if ($result !== true) {
		...
		return Customweb_Core_Http_Response::redirect(Mage::getUrl('BNPMercanetCw/Externalcheckout/login', array(
			'_secure' => true
		)));
	}
	...
	public function renderShippingMethodSelectionPane(Customweb_Payment_ExternalCheckout_IContext $context, $errorMessages){
		...
		$block = Mage::getSingleton('core/layout')->createBlock('bnpmercanetcw/externalCheckout_shippingMethods')->setContext($context);
		return $block->toHtml();
	}
	...
	public function getPossiblePaymentMethods(Customweb_Payment_ExternalCheckout_IContext $context){
		...
		foreach ($activePaymentMethods as $paymentMethodCode => $paymentMethod) {
			if (strpos($paymentMethodCode, 'bnpmercanetcw') === 0) {
	...
	public function renderReviewPane(Customweb_Payment_ExternalCheckout_IContext $context, $renderConfirmationFormElements, $errorMessage){
		...
		$update->load('bnpmercanetcw_external_checkout_review');
		$layout->generateXml();
	...
	public function validateReviewForm(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request){
		...
		$helper = Mage::helper('BNPMercanetCw');

		$quote = $context->getQuote();
		if ($context->hasBasketChanged($quote, $request)) {
			$context->updateFromQuote($quote);
			$context->save();
			throw new Exception(Mage::helper('BNPMercanetCw')->__('The shopping cart has been altered!'));
		}
	...
	public function renderAdditionalFormElements(Customweb_Payment_ExternalCheckout_IContext $context, $errorMessage){
		...
		$block = Mage::getSingleton('core/layout')->createBlock('bnpmercanetcw/externalCheckout_additionalInformation')->setContext($context);
		return $block->toHtml();
	}
	...
	public function processAdditionalFormElements(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request){
		...
		if (Mage::helper('BNPMercanetCw/externalCheckout')->isGenderRequired($context->getQuote())) {
		...
		if (Mage::helper('BNPMercanetCw/externalCheckout')->isDateOfBirthRequired($context->getQuote())) {
	...
	public function createOrder(Customweb_Payment_ExternalCheckout_IContext $context){
		...
		try {
			...
			Mage::helper('BNPMercanetCw')->logException($e);
			throw $e;
		}
	}
	...
	private function prepareGuestQuote(Customweb_BNPMercanetCw_Model_ExternalCheckoutContext $context){
		$quote = $context->getQuote();
	...
	private function prepareNewCustomerQuote(Customweb_BNPMercanetCw_Model_ExternalCheckoutContext $context){
		$quote = $context->getQuote();
	...
	private function prepareCustomerQuote(Customweb_BNPMercanetCw_Model_ExternalCheckoutContext $context){
		$quote = $context->getQuote();
		$billing = $quote->getBillingAddress();
		$shipping = $quote->isVirtual() ? null : $quote->getShippingAddress();
	...
	private function involveNewCustomer(Customweb_BNPMercanetCw_Model_ExternalCheckoutContext $context){
		$customer = $context->getQuote()->getCustomer();
		Mage::getSingleton('customer/session')->loginById($customer->getId());
	}

	private function refreshContext(Customweb_BNPMercanetCw_Model_ExternalCheckoutContext $context){
		if (!($context instanceof Customweb_BNPMercanetCw_Model_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('Customweb_BNPMercanetCw_Model_ExternalCheckoutContext');
		}
...
// 			}
		...
		if (!empty($paymentMethod)) {
			...
			$data = array(
				'method' => 'bnpmercanetcw_' . $paymentMethod,
				'checks' => 4 | 1 | 2 | 32 | 128,
			);

			$quote->setBNPMercanetCwExternalCheckout(true);
		...
		if ($quote->isVirtual()) {
			$context->setShippingMethodName(Mage::helper('BNPMercanetCw')->__('No shipping method needed.'));
		} else {
		...
		if ($context->getBillingAddress() !== null && $context->getBillingAddress() instanceof Customweb_Payment_Authorization_OrderContext_IAddress) {
			if (!in_array($context->getBillingAddress()->getCountryIsoCode(), $allowCountries)) {
				throw new Exception(Mage::helper('BNPMercanetCw')->__('It is not possible to checkout in your country.'));
			}
		}
	...
	private function updateUserSessionWithCurrentUser(Customweb_BNPMercanetCw_Model_ExternalCheckoutContext $context){
		if (!($context instanceof Customweb_BNPMercanetCw_Model_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('Customweb_BNPMercanetCw_Model_ExternalCheckoutContext');
		}
		...
		$quote = $context->getQuote();
		if ($customer->getId()) {
			Mage::register('bnpmercanetcw_external_checkout_login', true);
			Mage::getSingleton('customer/session')->setCustomerAsLoggedIn($customer);
			$quote->setCheckoutMethod(Mage_Checkout_Model_Type_Onepage::METHOD_CUSTOMER)->save();
			Mage::unregister('bnpmercanetcw_external_checkout_login');
		}
