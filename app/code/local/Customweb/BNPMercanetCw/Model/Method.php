...
new Customweb_BNPMercanetCw_Model_TranslationResolver();

abstract class Customweb_BNPMercanetCw_Model_Method extends Mage_Payment_Model_Method_Abstract implements
		Customweb_Payment_Authorization_IPaymentMethod {
	protected $_code = 'bnpmercanetcw';
	protected $_formBlockType = 'bnpmercanetcw/form';
	protected $_infoBlockType = 'bnpmercanetcw/info';

	/**
	 * Order statuses        	   				 	 	   
	 */
	const BNPMERCANETCW_STATUS_PENDING = 'pending_bnpmercanetcw';
	const BNPMERCANETCW_STATUS_CANCELED = 'canceled_bnpmercanetcw';
	...
	protected function setOrderStatus(&$order, $status, $logEntry = null){
		try {
			...
			Mage::dispatchEvent('bnpmercanetcw_order_status', array(
				'order' => $order,
				'status' => $status
			));
		}
	...
	public function isAvailable($quote = null){
		...
		if ($isAvailable && $quote !== null && $quote->getBNPMercanetCwExternalCheckout() !== true) {
	 ...
	 * @return Customweb_BNPMercanetCw_Model_Method
	 */
	public function initialize($paymentAction, $stateObject){
		$stateObject->setStatus(self::BNPMERCANETCW_STATUS_PENDING);
	...
	public function validate(){
		...
		$arguments = null;
		return Customweb_Licensing_BNPMercanetCw_License::run('an5akd5hmuj5vps2', $this, $arguments);
	}

	final public function call_6f3u39c4sjjrfqn7() {
	 ...
	 * @return string
	 */
	public function processServerAuthorization(Customweb_BNPMercanetCw_Model_Transaction $transaction, array $parameters){
	...
	public function cancel(Varien_Object $payment){
		...
		try {
			if (Mage::registry('bnpmercanetcw_update_transaction') != null) {
				$transaction = Mage::registry('bnpmercanetcw_update_transaction');
			}
			else {
				$transaction = $this->getHelper()->loadTransactionByPayment($payment->getId());
			}
			$order = $payment->getOrder();
			Customweb_BNPMercanetCw_Model_ConfigurationAdapter::setStore($order);
			...
			if (!$transaction->getTransactionObject()->isCancelled() && $order->getStatus() != self::BNPMERCANETCW_STATUS_PENDING) {
				$this->getHelper()->log(
	 ...
	 * @param float $amount
	 *
	 * @return Mage_BNPMercanetCw_Model_Method
	 */
	public function refund(Varien_Object $payment, $amount){
		...
		try {
			...
			$refunded = false;
			if ($transaction->getTransactionObject()->isRefundPossible() && Mage::registry('cw_bnpmercanetcw_refund_update') !== true) {
			...
			if ($refunded || Mage::registry('cw_bnpmercanetcw_refund_update') === true) {
				...
				$payment->getCreditmemo()->setBnpmercanetcwRefundId($refund->getRefundId());
	...
	protected function getInvoiceItems($invoice, $amount, $currency, $useBaseCurrency = false){
		...
		$invoiceItems = Mage::getModel('bnpmercanetcw/invoiceItems');
	 ...
	 */
	public function processPayment(Mage_Sales_Model_Order $order, Customweb_BNPMercanetCw_Model_Transaction $transaction){
		if ($order->getId()) {
			Customweb_BNPMercanetCw_Model_ConfigurationAdapter::setStore($order);
			Mage::getSingleton('core/session')->unsetBNPMercanetCwTransactionId();
			if ($transaction->getTransactionObject()->isAuthorized()) {
				$transaction->saveIgnoreOrderStatus();
				$order->setState(self::STATE_PROCESSING);
				$order->save();

				Mage::dispatchEvent('bnpmercanetcw_payment_success', array(
					'order' => $order
				));
				...
				if ($transaction->getTransactionObject()->isAuthorizationUncertain()) {
					...
					$order->addStatusHistoryComment(
							$this->getHelper()->__('Customer successfully returned from BNP Paribas Mercanet') . "\n<br>PaymentId:" .
									 $transaction->getTransactionObject()->getPaymentId());
					$order->save();
					$this->createAuthPaymentTransaction($order, $transaction);
					if ($this->createInvoice($order, $transaction)) {
						...
						Mage::dispatchEvent('bnpmercanetcw_invoice_created', array(
							'order' => $order
						));
					}
				}
	 ...
	 *
	 * @param Customweb_BNPMercanetCw_Model_Transaction $transaction
	 */
	public function createInvoiceAfterUncertain(Customweb_BNPMercanetCw_Model_Transaction $transaction){
	...
	protected function createAuthPaymentTransaction(Mage_Sales_Model_Order $order, Customweb_BNPMercanetCw_Model_Transaction $transaction){
	 ...
	 */
	public function createInvoice(Mage_Sales_Model_Order $order, Customweb_BNPMercanetCw_Model_Transaction $transaction){
		$result = false;
	...
	public function getOrderPlaceRedirectUrl(){
		...
		if ($adapter instanceof Customweb_Payment_Authorization_PaymentPage_IAdapter) {
			return Mage::getUrl('BNPMercanetCw/process/ppRedirect',
					array(
		...
		elseif ($adapter instanceof Customweb_Payment_Authorization_Iframe_IAdapter ||
			...
			return Mage::getUrl('BNPMercanetCw/checkout/pay',
					array(
						'transaction_id' => $transactionId,
						'secret' => $secretHash,
						'_secure' => true
			...
			return Mage::getUrl('BNPMercanetCw/process/dummy', array(
				'_secure' => true
			));
		}
	}

	public function generateHiddenFormParameters(Customweb_BNPMercanetCw_Model_Transaction $transaction){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
	...
	public function getFormRenderer(){
		$renderer = new Customweb_BNPMercanetCw_Model_FormRenderer();
		$renderer->setNameSpacePrefix($this->getCode());
		return $renderer;
	}

	public function generateJavascriptForAjax(Customweb_BNPMercanetCw_Model_Transaction $transaction){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
	...
	public function fail(Customweb_BNPMercanetCw_Model_Transaction $transaction, array $parameters){
		$order = $transaction->getOrder();

		Mage::dispatchEvent('bnpmercanetcw_payment_fail', array(
			'order' => $order
		));
		...
		if ($order->getState() != Mage_Sales_Model_Order::STATE_CANCELED &&
			...
			//$order->setIsActive(0);
			$this->setOrderStatus($order, Customweb_BNPMercanetCw_Model_Method::BNPMERCANETCW_STATUS_CANCELED,
	...
	public function success(Customweb_BNPMercanetCw_Model_Transaction $transaction, array $parameters){
		$quote = Mage::getModel('sales/quote');
	...
	public function pullUpdate(Customweb_BNPMercanetCw_Model_Transaction $transaction){
	...
	public function redirectToPaymentPage(Customweb_BNPMercanetCw_Model_Transaction $transaction, array $parameters){
		$info = $this->getInfoInstance();
	...
	public function getFormHtml($adapter, Customweb_BNPMercanetCw_Model_Transaction $transaction, $javaScript = true, $additionalFields = "", $formData = null){
		if ($formData === null) {
			$formData = $_REQUEST;
		}
	...
	public function startMotoAuthorization(Mage_Sales_Model_Order $order, array $parameters){
		...
		$orderContext = Customweb_BNPMercanetCw_Model_OrderContext::fromOrder($order);
	...
	public function generateMotoForm(Mage_Sales_Model_Order $order, Customweb_BNPMercanetCw_Model_Transaction $transaction, $parameters){
		$adapter = $this->getHelper()->getAuthorizationAdapter(Customweb_Payment_Authorization_Moto_IAdapter::AUTHORIZATION_METHOD_NAME);
		$failedTransaction = $this->loadFailedTransaction();
		$paymentCustomerContext = $this->getHelper()->getPaymentCustomerContext($order->getCustomerId());
		...
		$renderer = new Customweb_BNPMercanetCw_Model_BackendFormRenderer();
	...
	public function getIFrameUrl(Customweb_BNPMercanetCw_Model_Transaction $transaction, array $parameters){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
		$url = "";
	...
	public function getIFrameHeight(Customweb_BNPMercanetCw_Model_Transaction $transaction, array $parameters){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
		$height = 1000;
	...
	public function getWidgetHtml(Customweb_BNPMercanetCw_Model_Transaction $transaction, array $parameters){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
		$url = "";
	...
	public function getPaymentMethodConfigurationValue($key, $languageCode = null){
		$result = null;
		$rs = Mage::getStoreConfig('payment/' . $this->getCode() . '/' . $key, Customweb_BNPMercanetCw_Model_ConfigurationAdapter::getStoreId());
		...
		elseif (isset($fileKeys[$key])) {
			...
			if (empty($rs) || $defaultValue == $rs) {
				return Mage::helper('BNPMercanetCw')->getAssetResolver()->resolveAssetStream($defaultValue);
			}
			else {
				return new Customweb_Core_Stream_Input_File(
						Mage::getBaseDir('media') . '/bnpmercanetcw/setting/' . $this->getPaymentMethodName() . '/' . $key . '/' . $rs);
			}
		}
		else {
			$result = $rs;
	...
	public function existsPaymentMethodConfigurationValue($key, $languageCode = null){
		...
		return null != Mage::getStoreConfig('payment/' . $this->getCode() . '/' . $key,
				Customweb_BNPMercanetCw_Model_ConfigurationAdapter::getStoreId());
	}
	 ...
	 * @return NULL
	 */
	public function getAuthorizationAdapter($isOrderAvailable, Customweb_BNPMercanetCw_Model_Transaction $transaction = null){
	...
	public function getOrderContext($isOrderAvailable, $quote = null){
		if ($this->orderContext == null) {
			$orderContext = new Customweb_BNPMercanetCw_Model_OrderContext($this, $isOrderAvailable, null, $quote);
	...
	public function createTransaction(Mage_Sales_Model_Order $order, $moto = false){
		$transaction = Mage::getModel('bnpmercanetcw/transaction');
		$transaction->save();

		Mage::getSingleton('core/session')->setBNPMercanetCwTransactionId($transaction->getId());
	 ...
	 * @return Customweb_BNPMercanetCw_Model_Transaction
	 */
	public function updateTransaction(Customweb_BNPMercanetCw_Model_Transaction $transaction, Mage_Sales_Model_Order $order, $moto = false){
		if (!$transaction->getOrderId() && $order->getId()) {
			...
			$session->unsBNPMercanetCwCheckoutId();
	 ...
	 * @return Customweb_BNPMercanetCw_Model_TransactionContext
	 */
	public function getTransactionContext(Mage_Sales_Model_Order $order, Customweb_BNPMercanetCw_Model_Transaction $transaction, $backendSuccessUrl = '', $backendFailUrl = ''){
		$aliasTransactionId = Mage::getSingleton('checkout/session')->getAliasId();
		$storeId = $order->getStore()->getId();
		$orderContext = Customweb_BNPMercanetCw_Model_OrderContext::fromOrder($order);
		$transactionContext = new Customweb_BNPMercanetCw_Model_TransactionContext($orderContext, $order->getIncrementId(),
	 ...
	 * @return Customweb_BNPMercanetCw_Model_TransactionContext
	 */
	public function getMotoTransactionContext(Mage_Sales_Model_Order $order, Customweb_BNPMercanetCw_Model_Transaction $transaction){
		$storeId = $order->getStore()->getId();
		$orderContext = Customweb_BNPMercanetCw_Model_OrderContext::fromOrder($order);

		$backendSuccessUrl = Mage::getModel('adminhtml/url')->getUrl('adminhtml/motobnpmercanetcw/success',
				array(
					'order_id' => $order->getId()
				));
		$backendFailUrl = Mage::getModel('adminhtml/url')->getUrl('adminhtml/motobnpmercanetcw/fail', array(
			'order_id' => $order->getId()
		));
	...
	public function getHelper(){
		if ($this->helper == null) {
			$this->helper = Mage::helper('BNPMercanetCw');
		}
		return $this->helper;
	}
	 ...
	 */
	protected function getErrorMessageToDisplay(Customweb_BNPMercanetCw_Model_Transaction $transaction){
	...
	public function loadAliasForCustomer(){
		...
		if (Mage::helper('customer')->isLoggedIn()) {
			...
			$collection = Mage::getModel('bnpmercanetcw/transaction')->getCollection()->addFieldToFilter('customer_id',
					array(
						'eq' => $customerId
