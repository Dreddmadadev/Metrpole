 ...
 * @Bean
 */
class Customweb_BNPMercanetCw_Model_BackendOperation_RefundAdapter implements Customweb_Payment_BackendOperation_Adapter_Shop_IRefund
{
	public function refund(Customweb_Payment_Authorization_ITransaction $transaction)
	{
		...
		Mage::register('bnpmercanetcw_update_transaction', $transactionModel);
	...
	protected function createCreditmemo(Mage_Sales_Model_Order $order, $data)
	{
		...
		if ($creditmemo) {
			if (($creditmemo->getGrandTotal() <=0) && (!$creditmemo->getAllowZeroGrandTotal())) {
				Mage::throwException(
					Mage::helper('BNPMercanetCw')->__("Credit memo's total must be positive.")
				);
			}

			Mage::register('cw_bnpmercanetcw_refund_update', true);
