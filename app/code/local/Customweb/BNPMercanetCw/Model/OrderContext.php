...
class Customweb_BNPMercanetCw_Model_OrderContext extends Customweb_Payment_Authorization_OrderContext_AbstractDeprecated implements Customweb_Payment_Authorization_IOrderContext
{
	/**
	 * Transient fields
	 */
	...
	/**
	 *
	 * @param Customweb_BNPMercanetCw_Model_Method $paymentMethod
	 * @param unknown $isOrderAvailable
	 */
	public function __construct(Customweb_BNPMercanetCw_Model_Method $paymentMethod, $isOrderAvailable, $storeId = null, $quote = null)
	{
		...
		$checkoutId = $session->getBNPMercanetCwCheckoutId();
		if($checkoutId === null) {
			$checkoutId = Customweb_Core_Util_Rand::getUuid();
			$session->setBNPMercanetCwCheckoutId($checkoutId);
		}
	...
	{
		$orderContext = new Customweb_BNPMercanetCw_Model_OrderContext($order->getPayment()
	...
	{
		return Mage::getModel('bnpmercanetcw/invoiceItems')->getInvoiceItems($this->getOrderQuote(), $this->getStore()->getStoreId(), $this->useBaseCurrency());
	}
