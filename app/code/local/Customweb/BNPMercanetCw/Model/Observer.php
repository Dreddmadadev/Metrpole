...
class Customweb_BNPMercanetCw_Model_Observer
{
	private $timeout = 0;

	public function initCart(Varien_Event_Observer $observer)
	{
		if (Mage::getStoreConfig('bnpmercanetcw/general/cancel_existing_orders')) {
			...
			$orders = Mage::getResourceModel('sales/order_collection')
				...
				->addAttributeToFilter('status', Customweb_BNPMercanetCw_Model_Method::BNPMERCANETCW_STATUS_PENDING)
				->load();
				...
				foreach ($productIds as $productId) {
					...
					if (!$product->isSalable()) {
						foreach ($orders as $order) {
							...
							$order->setIsActive(0);
							$order->addStatusToHistory(Customweb_BNPMercanetCw_Model_Method::BNPMERCANETCW_STATUS_CANCELED, Mage::helper('BNPMercanetCw')->__('Order cancelled, because the ordered products are not available.'));
							$order->save();
						}
						break;
					}
	...
	public function placeOrder(Varien_Event_Observer $observer)
	{
		...
		try {
			if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'bnpmercanetcw') === 0) {
		...
		try {
			if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'bnpmercanetcw') === 0) {
				...
				if (Mage::registry('cw_is_moto') == null && !empty($sessionTransactionId)) {
					$transaction = Mage::helper('BNPMercanetCw')->loadTransaction($sessionTransactionId);
	...
	public function cancelOrder(Varien_Event_Observer $observer)
	{
		$order = $observer->getOrder();
		if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'bnpmercanetcw') === 0) {
			$order->addStatusHistoryComment(Mage::helper('BNPMercanetCw')->__('Transaction cancelled successfully'));
		}
	}
	...
	public function invoiceView(Varien_Event_Observer $observer)
	{
		...
		if (strpos($invoice->getOrder()->getPayment()->getMethodInstance()->getCode(), 'bnpmercanetcw') === 0) {
			$transaction = Mage::helper('BNPMercanetCw')->loadTransactionByOrder($invoice->getOrder()->getId());
			...
			if (Mage::getSingleton('admin/session')->isAllowed('sales/order/actions/edit')
				...
				$block->addButton('edit', array(
					...
					'class'     => 'go',
					'onclick'   => 'setLocation(\''.$block->getUrl('*/editbnpmercanetcw/index', array('invoice_id'=>$invoice->getId())).'\')'
				));
			}
		}
	}
	...
	{
		if (Mage::registry('bnpmercanetcw_external_checkout_login') === true) {
	...
	public function collectExternalCheckoutWidgets(Varien_Event_Observer $observer)
	{
		...
		$widgets = array_merge($widgets, Mage::getModel('bnpmercanetcw/externalCheckoutWidgets')->getWidgets());
		$observer->getEvent()->setWidgets($widgets);
	}

	public function registerTranslationResolver(Varien_Event_Observer $observer)
	{
		if (!Mage::registry('customweb_bnpmercanetcw_transaction_resolver_registered')) {
			Customweb_I18n_Translation::getInstance()->addResolver(new Customweb_BNPMercanetCw_Model_TranslationResolver());
			Mage::register('customweb_bnpmercanetcw_transaction_resolver_registered', true);
		}
	}

	public function registerLoggingListener(Varien_Event_Observer $observer)
	{
		if (!Mage::registry('customweb_bnpmercanetcw_logging_listener_registered')) {
			Customweb_Core_Logger_Factory::addListener(new Customweb_BNPMercanetCw_Model_LoggingListener());
			Mage::register('customweb_bnpmercanetcw_logging_listener_registered', true);
		}
	}
	...
	public function migrateCustomerTransactions(Varien_Event_Observer $observer)
	{
		...
		Mage::helper('BNPMercanetCw')->migrateCustomersTransactions($customer);
	}
}
