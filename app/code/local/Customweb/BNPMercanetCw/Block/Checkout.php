 ...
 * 
 */
class Customweb_BNPMercanetCw_Block_Checkout extends Mage_Core_Block_Template {

	protected function _construct(){
		parent::_construct();
		$this->setTemplate('customweb/bnpmercanetcw/checkout.phtml');
	}
	...
	public function getPaymentMethods(){
		...
		foreach ($payments as $paymentCode => $paymentModel) {
			if (preg_match("/bnpmercanetcw/i", $paymentCode)) {
	...
	public function getAuthorizeUrl(){
		return Mage::getUrl('BNPMercanetCw/process/authorize', array(
			'_secure' => true 
		));
	}
	...
	public function getSaveShippingMethodUrl(){
		return Mage::getUrl('BNPMercanetCw/onepage/saveShippingMethod', array(
			'_secure' => true 
		));
	}
	...
	public function getPreloadUrl(){
		if (version_compare(Mage::getVersion(), '1.8', '>=')) {
			return Mage::getUrl('BNPMercanetCw/process/preloadOnepage', array(
				'_secure' => true 
			));
		}
		else {
