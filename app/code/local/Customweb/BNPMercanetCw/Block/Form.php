 ...
 *
 */
class Customweb_BNPMercanetCw_Block_Form extends Mage_Payment_Block_Form {
	private $printJavascript = true;

	protected function _construct(){
		parent::_construct();
		$this->setTemplate('customweb/bnpmercanetcw/form.phtml');
	}

	public function getContent(){
		
		$arguments = null;
		return Customweb_Licensing_BNPMercanetCw_License::run('t0j554797fsnrjts', $this, $arguments);
	}

	final public function call_taato3grtmar3q24() {
		...
		$form = '
			<div id="payment_description_' . $code . '" class="cw_payment_description">
				' . ($method->getPaymentMethodConfigurationValue('show_image') ? '
					<img src="' . $this->getSkinUrl('images/bnpmercanetcw/' . $method->getPaymentMethodName() . '.png') . '" /><br/>
	...
	public function getProcessUrl(){
		return Mage::getUrl('BNPMercanetCw/process/process', array(
			'_secure' => true
		));
	}

	public function getJavascriptUrl(){
		return Mage::getUrl('BNPMercanetCw/process/ajax', array(
			'_secure' => true
		));
	}

	public function getHiddenFieldsUrl(){
		return Mage::getUrl('BNPMercanetCw/process/getHiddenFields', array(
			'_secure' => true
		));
	}

	public function getVisibleFieldsUrl(){
		return Mage::getUrl('BNPMercanetCw/process/getVisibleFields', array(
			'_secure' => true
		));
	}
	...
	public function getAliasSelect(){
		...
		if ($payment->getPaymentMethodConfigurationValue('alias_manager') == 'active') {
			...
			if (count($aliasList)) {
				$alias = array(
					'new' => Mage::helper('BNPMercanetCw')->__('New card')
				);
				...
				// on select fields.        	   				 	 	   
				$selectControl = new Customweb_BNPMercanetCw_Model_Select("alias_select", $alias, 'new',
						"cwpm_" . $payment->getCode() . ".loadAliasData(this)");
				$aliasElement = new Customweb_Form_Element(Mage::helper('BNPMercanetCw')->__("Saved cards:"), $selectControl,
						Mage::helper('BNPMercanetCw')->__("You may choose one of the cards you paid before on this site."));
