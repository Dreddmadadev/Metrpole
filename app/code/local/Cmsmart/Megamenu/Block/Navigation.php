...
//        $head->addItem('skin_css', 'css/cmsmart/megamenu/megamenu.css');
   ...
   protected function _renderCategoryMenuItemHtml($category, $level = 0, $isLast = false, $isFirst = false,
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false, $countSameLevel = 5)
    {
        ...
        if($showthumbnail == 1){
			if ($imgaes) {
				$html[] = '<img class="thumbnail imgcateg" data-src="' . $imgaes .'" style="float: left;z-index: 1" alt="'.$category->getData("url_key").'" />' ;
			}else {
    ...
    public function getBlockTop($block){
        $active_static_block = isset($block[0])?$block[0]['active_static_block']:0;
        $active_static_block_top = isset($block[0])?$block[0]['active_static_block_top']:0;
        $idblock = isset($block[0])?$block[0]['static_block_top']:0;
    ...
    public function getBlockLeft($block){
        $active_static_block = isset($block[0])?$block[0]['active_static_block']:0;
        $active_static_block_left = isset($block[0])?$block[0]['active_static_block_left']:0;
        $id =  isset($block[0])?$block[0]['category_id']:0;
        $idblock = isset($block[0])?$block[0]['static_block_left']:0;
    ...
    public function getBlockRight($block){
        $active_static_block = isset($block[0])?$block[0]['active_static_block']:0;
        $active_static_block_right = isset($block[0])?$block[0]['active_static_block_right']:0;
        $idblock = isset($block[0])?$block[0]['static_block_right']:0;
        $id =  isset($block[0])?$block[0]['category_id']:0;
    ...
    public function getBlockBottom($block){
        $active_static_block = isset($block[0])?$block[0]['active_static_block']:0;
        $active_static_block_bottom = isset($block[0])?$block[0]['active_static_block_bottom']:0;
        $idblock = isset($block[0])?$block[0]['static_block_bottom']:0;
    ...
    public function widthp($idc){
        ...
        $cent = ($widthchildrenli != 0)?((($column)*30)/$widthchildrenli)*100 : 0;
    ...
    public function getLabel($megamenu){
         if(isset($megamenu[0]) && $megamenu[0]['active_label'] == 1){
    ...
    public function getLevle($id){
        $megamenu = Mage::helper('megamenu')->Megamenu($id);
         if(isset($megamenu[0]) && $megamenu[0]['level_column_count'] != 0){
    ...
    public function fontcolor($megamenu){
        if(isset($megamenu[0]) && $megamenu[0]['font-color']) {
    ...
    public function backgroundcolor($megamenu){
        if(isset($megamenu[0]) && $megamenu[0]['background-color']) {
