...
class Amasty_Optimization_Model_Image_Optimizer
{
    ...
    const JPEGOPTIM = 'jpegoptim --all-progressive --strip-xmp --strip-com --strip-exif --strip-iptc %f';
    const JPEGOPTIM_90 = 'jpegoptim --all-progressive --strip-all -m90 %f';
    const JPEGOPTIM_80 = 'jpegoptim --all-progressive --strip-all -m80 %f';
    const OPTIPNG = 'optipng -o7 -strip all %f';
    const GIFSICLE = 'gifsicle -O3 %f -o %f';
    const NOTHING = '';
    ...
    public function __construct(array $args = array())
    {
        ...
        foreach (array('png', 'jpeg', 'gif') as $extension) {
            ...
            $command = constant('self::'. $command);
            if ($command) {
                if (false === strpos($command, '%f')) {
                    $this->logger->info("File placeholder '%f' is not found in command for image type '$extension'");
                } else {
                    $this->commands[$extension] = $command;
                }
            } else {
                $this->logger->info("No command specified for image type '$extension'");
            }
        }
    }
    ...
    public function optimizeImage($path)
    {
        ...
        if ($type && array_key_exists($type, $this->commands)) {
