...
class Mirasvit_FeedExport_Model_Feed_Generator_Pattern extends Varien_Object
{
    ...
    public function parsePattern($pattern)
    {
        ...
        if (!isset(self::$_patterns[$pattern])) {
            ...
            if (isset($matches[1])) {
                ...
                //type B {...}
                preg_match('/{([^},|]+)([|])?(only_parent|parent_if_empty|parent|grouped|salable_grouped|configurable|bundle)?([^}]*)}/', $pattern, $matches);
                if (!isset($matches[1])) {
                    return false;
                }
                $key = $matches[1];
    ...
    {
        preg_match_all('/{([^}]+)(\sonly_parent|\sparent_if_empty|\sparent|\sgrouped|\ssalable_grouped|\sconfigurable|\sbundle)?([^}]*)}/', $content, $matches);
